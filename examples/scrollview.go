...
func (item Item6) Text() string    { return item.T }
func (item Item6) ImageIndex() int { return 1 }
...
func scrollView() {
	...
	mainWindow.SetText("ScrollView Demo")
	...
	if err := tree.InsertItem(&Item6{"Third"}, p, nil); err != nil {
		panic(err)
	}
	if err := tree.InsertItem(&Item6{"Fourth"}, p, nil); err != nil {
		panic(err)
	}
	for i := 0; i < 50; i++ {
		if err := tree.InsertItem(&Item6{"after second"}, sec, nil); err != nil {
			panic(err)
		}
	}
	tree.Expand(p)
