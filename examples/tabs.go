...
func (item Item4) Text() []string    { return item.T }
func (item *Item4) SetText(s string) { item.T[0] = s }

func (item Item4) Checked() bool            { return item.checked }
func (item *Item4) SetChecked(checked bool) { item.checked = checked }
func (item Item4) ImageIndex() int          { return 0 }
...
func tabs() {
	...
	ls.SetPos(10, 180)
	p1 := &Item4{[]string{"First Item", "A"}, true}
	ls.AddItem(p1)
	p2 := &Item4{[]string{"Second Item", "B"}, true}
	ls.AddItem(p2)
	p3 := &Item4{[]string{"Third Item", "C"}, true}
	ls.AddItem(p3)
	for i := 0; i < 200; i++ {
		p4 := &Item4{[]string{"Fourth Item", "D"}, false}
		ls.AddItem(p4)
	}
